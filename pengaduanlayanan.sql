...
CREATE TABLE
	IF NOT EXISTS "kepuasan" (
		...
		PRIMARY KEY ("id"),
		CONSTRAINT "chk_q1" CHECK (
			(
				(q1 >= 1)
				AND (q1 <= 4)
			)
		),
		CONSTRAINT "chk_q2" CHECK (
			(
				(q2 >= 1)
				AND (q2 <= 4)
			)
		),
		CONSTRAINT "chk_q3" CHECK (
			(
				(q3 >= 1)
				AND (q3 <= 4)
			)
		),
		CONSTRAINT "chk_q4" CHECK (
			(
				(q4 >= 1)
				AND (q4 <= 4)
			)
		),
		CONSTRAINT "chk_q5" CHECK (
			(
				(q5 >= 1)
				AND (q5 <= 4)
			)
		),
		CONSTRAINT "chk_q6" CHECK (
			(
				(q6 >= 1)
				AND (q6 <= 4)
			)
		),
		CONSTRAINT "chk_q7" CHECK (
			(
				(q7 >= 1)
				AND (q7 <= 4)
			)
		),
		CONSTRAINT "chk_q8" CHECK (
			(
				(q8 >= 1)
				AND (q8 <= 4)
			)
		),
		CONSTRAINT "chk_q9" CHECK (
			(
				(q9 >= 1)
				AND (q9 <= 4)
			)
...
VALUES
	...
	(
		5,
		'Bagaimana pendapat Anda tentang kewajaran biaya/tarif dalam pelayanan? (Jika peserta BPJS/Asuransi tidak perlu diisi)'
	),
	(
		6,
